			...
			success:function(data){
				...
				for(var i = 0; i < data.length; i++){
					...
					str += ''
						...
						+ '</a><div class="row">'
						<% if (admin) { %>
						+ '<div class="col-6 text-end"><i class="btn bi bi-trash3-fill" onclick="photoRemove(\''+data[i].gno+'\')"></i></div>'
						<% } else {%>
						+ '<br>'
						<% } %>
						+ '</div></div>';
...
function photoRemove(gno) {
	if (confirm('정말로 삭제 하시겠습니까?')){
		location.href="galleryDel.jsp?gno=" + gno;
	}
}
